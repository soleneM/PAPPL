...
/** 
 *  Pour une période donnée liste par commune des espèces renseignées avec le nombre des témoignages de ces espèces
 */
...
/** 
 *  Pour une période donnée liste par département des espèces renseignées avec le nombre des témoignages de ces espèces
 */
...
/** 
 *   liste chronologique des différents lieux prospectés et, dans ces lieux, des différentes espèces
 *   observées avec détail des nombres et stade/sexe
 */
		...
		if ((info.get("typeDonnees") != null) && (!info.get("typeDonnees").equals("null"))) {
			...
			switch (typeStat) {
				...
				case 110 : // Carnet de Chasse
					...
					excelData = ListeExportExcel.carnetDeChasseExcel(info,carnetDeChasse);
